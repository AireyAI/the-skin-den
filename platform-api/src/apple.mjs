...
async function fetchAppleKeys() {
  ...
  if (!res.ok) throw new Error("Could not fetch Apple JWKS");
...
export async function verifyAppleIdentityToken(idToken) {
  ...
  const aud = config.appleClientId;
  if (aud && payload.aud !== aud) throw new Error("Apple token audience mismatch");
...
/** Warm JWKS on boot so the first Sign in with Apple is not waiting on Apple CDN. */
export function warmAppleJwks() {
  fetchAppleKeys().catch(() => {});
}
